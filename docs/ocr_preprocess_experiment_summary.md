...
최종 채택 pipeline의 보고용 예상 처리시간은 모델 초기화와 실행 변동 여유를 포함해 약 **12.02분**이다. 저장된 패스별 시간만 합산한 값은 약 11.71분이며, 두 값 모두 40분 제한 안이다. 효과가 없었던 keyword crop은 최종 실행시간과 pipeline에서 제외한다.
